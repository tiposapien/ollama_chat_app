...
def delete_chat_log(filename):
    """Mark a chat log as deleted by renaming the file."""
    log_filepath = os.path.join(LOG_DIR, filename)
    if os.path.exists(log_filepath):
...
def handle_send_message(data):
    ...
    try:
        response = requests.post(f'{OLLAMA_API_BASE}/chat', json=obj_payload, stream=True, timeout=30)
        response.raise_for_status()
...
if __name__ == '__main__':
    # For local dev: socketio.run with debug from config
    socketio.run(app, debug=app.config['DEBUG_MODE'])